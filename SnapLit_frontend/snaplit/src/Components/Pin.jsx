...
const Pin = ({pin: {postedBy, image, _id, destination, save}}) => {
   ...
   const [savingPost, setSavingPost] = useState(false);
    ...
    const alreadySaved = !!(save?.filter((item) => item.postedBy._id === user.googgleId))?.length;
...
onClick={()=> navigate(`/pin-detail/${_id}`)}>
    <img src={urlFor(image).width(250).url()} className='rounded-lg wit-full' alt='user-post'/>
    {postHovered && (
        ...
        >
            ...
            <div class="flex jsutify-between items-center gap-2 w-full">
                {destination && (
                    <a href={destination}
                    target= "_blank"
                    rel= "noreferrer"
...
<Link to={`user-profile/${postedBy?._id}`} className='flex gap-2  mt-2 items-center'>
                 ...
                 <p className='font-semi-bold capitalize'>{postedBy?.name}</p>
</Link>
    </div>
  )
}
